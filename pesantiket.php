...
<html>
<head>
    <title>MyTicket | Pesan Tiket</title>
<?php
require 'koneksi.php';
	...
	<div class="tab-content">
		<div id="today" class="tab-pane fade show active">
			...
			function createTable()
			{
				...
				$sql_instance = "SELECT * FROM bus JOIN rute ON rute.ruteID=bus.ruteID WHERE tglKonser = CURDATE() ORDER BY bus.wktKonser ASC;";
                ...
                <table class="table">
                    ...
                    <td>' . $row["wktTiba"] . '</td>';
						if ($row['jumlah_kursi'] > 15) {
							echo '<td><a href="simpan-tiket.php?bis=' . $row["konserID"] . '" class="btn btn-success" role="button">Pesan</a></td>
                  </tr>';
						} elseif ($row['jumlah_kursi'] > 0) {
							echo '<td><a href="simpan-tiket.php?bis=' . $row["konserID"] . '" class="btn btn-warning" role="button">Pesan</a></td>
                  </tr>';
						} else {
							echo '<td><a href="simpan-tiket.php?bis=' . $row["konserID"] . '" class="btn btn-danger disabled" role="button">Habis</a></td>
                  </tr>';
						}
					}
		...
		<div id="tomorrow" class="tab-pane fade">
			...
			function createTable1()
			{
				...
				$row = $result->fetch_assoc();
				$userType = $row['kategoriID'];
				$sql_instance = "SELECT * FROM bus JOIN rute ON rute.ruteID=bus.ruteID WHERE tglKonser = CURDATE() + INTERVAL 1 DAY ORDER BY bus.wktKonser ASC;";
				$result = $koneksi->query($sql_instance);
				if (!$result) {
					trigger_error('Invalid query: ' . $koneksi->error);
				}
                ...
                <table class="table">
					...
					while ($row = $result->fetch_assoc()) {
						...
						<td>' . $row["tujuan"] . '</td>
						<td>' . $row["wktTiba"] . '</td>
										<td><a href="simpan-tiket.php?bis=' . $row["konserID"] . '" class="btn btn-success" role="button">Pesan</a></td>
						      </tr>';
					}
					echo '</tbody> </table> </div>';
				}
